...
async def lifespan(app: FastAPI):
    os.environ.setdefault("GEMINI_MODEL_NAME", "gemini-3.5-flash")
    yield
...
def run_analysis(job_id: str, text: str, source_name: str) -> None:
    """Blocking analysis — run in thread pool via BackgroundTasks."""
    from gemini_analyzer import analyze_novel_with_gemini
    ...
    try:
        ai_raw = analyze_novel_with_gemini(text)
        jobs[job_id]["progress"] = "AI 分析完成，正在构建剧本结构..."
        script_data = build_final_output(ai_raw, text, source_name)
...
async def analyze(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(run_analysis, job_id, request.text, request.source_name)
...
async def analyze_upload(
    ...
    background_tasks.add_task(run_analysis, job_id, text, name)
    return JobStatus(**jobs[job_id])
