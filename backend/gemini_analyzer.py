...
def analyze_novel_with_gemini(text: str) -> Dict[str, Any]:
    proxy_url = "http://127.0.0.1:7890"  # 如果你的 Clash/梯子是其他端口，请修改此处
...
def analyze_novel(
    ...
    """Route to RAG pipeline or full-text Gemini analysis."""
    if use_rag is None:
        use_rag = len(text) > 50000
    if use_rag:
        from rag.pipeline import analyze_novel_with_rag

        return analyze_novel_with_rag(
            text,
            progress_callback=progress_callback,
            reference_texts=reference_texts,
        )
    return analyze_novel_with_gemini(text)
